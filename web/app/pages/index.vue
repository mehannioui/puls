...
<template>
  <div class="min-h-screen font-sans" style="background: var(--bg-canvas); color: var(--fg-primary);">
    ...
    <!-- Nav -->
    <nav
      class="flex items-center justify-between px-4 md:px-8"
      style="height: 60px; border-bottom: 1px solid var(--border-subtle);"
    >
      <div class="flex items-center gap-4 md:gap-6">
        <PulseLogo :size="20"/>
        <span
          class="t-mono hidden sm:inline"
          style="font-size: 11px; padding: 2px 6px; border: 1px solid var(--border-subtle); border-radius: 4px; color: var(--fg-quiet);"
        >v2 · beta</span>
    ...
    <section class="px-4 md:px-8 mx-auto" style="max-width: 1180px; padding-top: 48px; padding-bottom: 40px;">
      ...
      <!-- Eyebrow pill -->
      <div
        class="inline-flex items-center gap-2 rounded-full mb-6"
        style="padding: 5px 10px 5px 6px; border: 1px solid var(--border-subtle); background: var(--bg-base); font-size: 12px;"
      >
      ...
      <div class="flex flex-wrap gap-3 items-center">
        <NuxtLink to="/app/services/new" class="pulse-btn primary lg">
          Start monitoring — free<PulseIcon name="chev" :size="12" color="var(--fg-on-accent)"/>
        </NuxtLink>
      ...
      <div class="pulse-card mt-10 md:mt-14" style="padding: 0; overflow: hidden;">
        <div
          class="flex items-center justify-between flex-wrap gap-4 px-4 md:px-5 py-4"
          style="border-bottom: 1px solid var(--border-subtle);"
        >
          <div class="flex items-center gap-3 md:gap-4">
            <span class="t-mono fg-ter flex items-center gap-[6px]" style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase;">
              <span class="pulse-dot op" style="width: 6px; height: 6px; box-shadow: none;"/>LIVE FEED
            </span>
          ...
          <div class="flex gap-5 md:gap-7">
            <div class="text-right">
              <div class="t-mono fg-pri" style="font-size: clamp(14px, 2.5vw, 18px);">99.998<span class="fg-qui" style="font-size: 11px;">%</span></div>
              <div class="t-eyebrow">UPTIME · 30d</div>
            </div>
            <div class="text-right">
              <div class="t-mono fg-pri" style="font-size: clamp(14px, 2.5vw, 18px);">142<span class="fg-qui" style="font-size: 11px;">ms</span></div>
              <div class="t-eyebrow">AVG · 24h</div>
            </div>
            <div class="text-right">
              <div class="t-mono fg-pri" style="font-size: clamp(14px, 2.5vw, 18px);">0</div>
              <div class="t-eyebrow">INCIDENTS</div>
            </div>
          </div>
        </div>
        <div
          class="relative"
          style="height: clamp(100px, 20vw, 160px); background: linear-gradient(180deg, var(--bg-base), oklch(0.16 0.008 240));"
        >
        ...
        <div class="grid px-4 md:px-5 py-3 gap-4 items-center" style="grid-template-columns: 1fr auto;">
          ...
          <div class="flex gap-6">
            <span class="t-mono fg-qui" style="font-size: 10px;">90 days</span>
            <span class="t-mono fg-qui" style="font-size: 10px;">today</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="px-4 md:px-8 mx-auto pb-10" style="max-width: 1180px;">
      ...
      <div class="flex flex-wrap justify-center md:justify-between items-center gap-6 px-0 md:px-10 opacity-50">
        <span v-for="b in ['Rivertide','Halfmoon','Stagework','Notari','Lumen','Patchgrid']" :key="b"
          class="t-mono fg-ter" style="font-size: 14px; letter-spacing: -0.01em;">{{ b }}</span>
      </div>
    </section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div
            ...
            style="width: 32px; height: 32px; background: var(--bg-raised); border: 1px solid var(--border-subtle); color: var(--pulse-brand);"
          >
          ...
          <div class="pt-3" style="border-top: 1px solid var(--border-subtle);">
            ...
            <div v-else-if="feat.footer.type === 'domain'" class="flex items-center gap-2">
              <span class="t-mono fg-sec" style="font-size: 11px;">status.yourapp.com</span>
              <span class="pulse-dot op"/>
            </div>
            <div v-else class="t-mono fg-sec flex items-center gap-[6px]" style="font-size: 11px;">
              <span class="pulse-dot outage"/>503 · webhooks · 13:38 UTC
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4" style="max-width: 820px;">
        <div class="pulse-card flex flex-col gap-4" style="padding: 24px;">
          <div class="flex items-center justify-between">
            <span class="t-h3">Free</span>
            <span class="t-mono fg-qui" style="font-size: 11px;">for indie devs</span>
          </div>
          <div class="flex items-baseline gap-1">
            <span class="t-mono" style="font-size: 40px; font-weight: 500;">$0</span>
            <span class="t-mono fg-ter">/forever</span>
          </div>
          ...
          <ul class="flex flex-col gap-2" style="list-style: none; padding: 0; margin: 0; font-size: 13px;">
            <li v-for="f in freePlan" :key="f" class="flex items-center gap-2">
              <PulseIcon name="check" :size="11" color="var(--pulse-brand)"/>
              <span class="fg-sec">{{ f }}</span>
            </li>
          </ul>
        </div>
        <div class="pulse-card flex flex-col gap-4 relative" style="padding: 24px; border-color: var(--pulse-brand-quiet);">
          ...
          <div class="flex items-center justify-between">
            <span class="t-h3">Pro</span>
            <span class="t-mono fg-qui" style="font-size: 11px;">for small teams</span>
          </div>
          <div class="flex items-baseline gap-1">
            <span class="t-mono" style="font-size: 40px; font-weight: 500;">$19</span>
            <span class="t-mono fg-ter">/month, flat</span>
          </div>
          <button class="pulse-btn primary" style="width: 100%; justify-content: center;">Start 14-day trial</button>
          <hr class="pulse-hr"/>
          <ul class="flex flex-col gap-2" style="list-style: none; padding: 0; margin: 0; font-size: 13px;">
            <li v-for="f in proPlan" :key="f" class="flex items-center gap-2">
              <PulseIcon name="check" :size="11" color="var(--pulse-brand)"/>
              <span class="fg-sec">{{ f }}</span>
            </li>
          </ul>
